...
const visibility_button = document.querySelectorAll('[data-toggle="article-visibility"]');
...
for (i = 0; i < visibility_button.length; i++){
    ...
    visibility_button[i].addEventListener("click", function(e){
        ...
        ajax.open("POST","article.php?action=article-visibility",true);
        ajax.send(formData);
    });
}
    ...
    delete_button[i].addEventListener("click", function(e){
        ...
        if (alert_box == null || alert_box == ""){
            ...
            if (alert_box == alias){
                ...
                ajax.open("POST","article.php?action=delete",true);
                ajax.send(formData);
