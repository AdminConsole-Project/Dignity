...
const index_button = document.querySelectorAll('[data-toggle="theme-main"]');
...
for (i = 0; i < index_button.length; i++){
    ...
    index_button[i].addEventListener("click", function(e){
        ...
        document.getElementById("ac-main-theme-info").children[0].classList.add("hide");
        document.getElementById("ac-main-theme-info").children[1].classList.add("hide");
        document.getElementById("ac-main-theme-info").children[2].classList.add("hide");
        ...
        if (alert_box == null || alert_box == ""){
            ...
            document.getElementById("ac-main-theme-info").children[1].classList.remove("hide");
            ...
            ajax.onload = function(){
                if (this.readyState == 4 && this.status == 200){
                    if (this.response == 1){
                        ...
                        document.getElementById("ac-main-theme-info").children[0].classList.remove("hide");
                        document.getElementById("ac-main-theme-info").children[0].children[0].setAttribute("aria-hidden","false");
                        window.location.reload();

                    }else if(this.response == 0){

                        document.getElementById("ac-main-theme-info").children[2].classList.remove("hide");
                        document.getElementById("ac-main-theme-info").children[2].children[0].setAttribute("aria-hidden","false");

                    }
                }
            };

            ajax.open("POST","themes.php?action=theme-main",true);
            ajax.send(formData);
        }
    });
}
...
for (i = 0; i < delete_button.length; i++){
    ...
    delete_button[i].addEventListener("click", function(e){
        ...
        let confirm_text = this.getAttribute("aria-text-confirmation");
        let prompt_text = this.getAttribute("aria-text");
        let alias = this.getAttribute("aria-alias");

        let confirm_box = confirm(confirm_text);

        if (confirm_box == null || confirm_box == ""){
            ...
            if (alert_box == null || alert_box == ""){
                ...
                if (alert_box == alias){
                    ...
                    ajax.open("POST","themes.php?action=delete",true);
                    ajax.send(formData);
