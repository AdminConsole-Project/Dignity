...
const visibility_button = document.querySelectorAll('[data-toggle="category-visibility"]');
...
for (i = 0; i < visibility_button.length; i++){
    ...
    visibility_button[i].addEventListener("click", function(e){
        ...
        ajax.open("POST","navigation.php?action=navigation-visibility",true);
        ajax.send(formData);
    });
}
    ...
    delete_button[i].addEventListener("click", function(e){
        ...
        if (alert_box == null || alert_box == ""){
            ...
            if (alert_box == alias){
                ...
                ajax.open("POST","navigation.php?action=delete",true);
                ajax.send(formData);
